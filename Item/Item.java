...
public class Item {
    ...
    public void adicionarItem(Itens item, ArrayList<Item> mochila) {
        ...
        for (Itens i : Itens.values()) {
            if (item == i) {
                for (int j = 0; j < mochila.size(); j++) {
                    if (item.equals(mochila.get(j)) && item.getStackable()) { // Confere se já consta na mochila e se é stackable
                        // Adicionar o item
    ...
    protected void removerItem(Itens item, ArrayList<Item> mochila) {
        for (int i = 0; i < mochila.size(); i++) {
            if (item.equals(mochila.get(i))) { // Se o item estiver dentro da mochila, pode ser removido
                // Remover o item
            }
        }
    }
    ...
    protected void consumirItem(Itens item) {
        for (int i = 0; i < mochila.size(); i++) {
            if (item.equals(mochila.get(i))) { // Se o item estiver presente na mochila
                if (item.getConsumivel()) { // Se o item for consumível
